...
$tpl = fopen(dirname(__FILE__)."/skin/default_EX/layout.php", "r");
$tpl_file = '';
while (!feof($tpl)){
...
掲示板が既にインストールされています。<br/><br/>
<a class="btn btn-danger" href="./install.setup.php">もどる</a>
</div>
<?php
exit();
...
<div id="installed" class="alert alert-error">
<a class="close" href="javascript:history.go(-1);">&times;</a>
DB情報が間違えています。もう一度確認してください。<br/><br/>
...
<table class="table table-bordered">
	<caption>掲示板インストール状態</caption>
	<thead>
		<tr >
			<th class="info">DB Table Name</th>
			<th class="info">状態</th>
			<th class="info">詳細</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>chibi_admin(掲示板)</td>
			<td>
			<?php
			if($db_check->admin==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->admin==false && empty($admin_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
			<td><?php echo "<p class=\"text-error\">".$admin_error."</p>";?></td>
		</tr>
		<tr>
			<td>chibi_skin(スキン)</td>
			<td>
			<?php
			if($db_check->skin==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->skin==false && empty($skin_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			...
			<td>
			<?php
			if($db_check->pic==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->pic==false && empty($pic_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			<td>chibi_comment(コメント)</td>
			<td>
			<?php
			if($db_check->comment==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->comment==false && empty($comment_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			<td>chibi_member(会員)</td>
			<td>
			<?php
			if($db_check->member==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->member==false && empty($member_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			...
			<td>
			<?php
			if($db_check->log==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->log==false && empty($log_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			<td>chibi_tpl(テンプレート)</td>
			<td>
			<?php
			if($db_check->tpl==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->tpl==false && empty($tpl_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			<td>chibi_emoticon(絵文字)</td>
			<td>
			<?php
			if($db_check->emoticon==true) echo "<p class=\"text-info\">テーブルが既に作成されています。</p>";
			else if($db_check->emoticon==false && empty($emoticon_error)==true) echo "<p class=\"text-success\">作成完了</p>";
			else echo "<p class=\"text-error\">エラー(詳細参考)</p>"
			?>
			</td>
		...
		<tr>
			...
			<?php if((empty($admin_error) && empty($skin_error) && empty($pic_error) && empty($comment_error) && empty($member_error) && empty($tpl_error) && empty($log_error))==true){
				echo "<a class=\"btn btn-primary\" href=\"admin/admin.php\">インストール完了</a>";
			}else{
				echo "<a class=\"btn btn-primary\" href=\"install.setup.php\">もどる</a><span class=\"help-block\">エラーが発生しました。エラー内容を確認してください。</span>";	
			}?>
			</td>
	</tbody>
</table>
