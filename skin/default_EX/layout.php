...
<div class="span8 offset2 text-center">
	<p>
		<form id="drawForm" method="POST" class="form-inline" action="./index.php?cid=<?=$cid?>&cAct=picDraw">
			...
			<span>横幅</span> <input type="text" class="input-mini tool_width" id="width" name="width" placeholder="<?=$bbs->op->pic_min_width?>-<?=$bbs->op->pic_max_width?>">
			<span>高さ</span> <input type="text" class="input-mini tool_height" id="height" name="height" placeholder="<?=$bbs->op->pic_min_width?>-<?=$bbs->op->pic_max_width?>">
...
<option selected="selected">ロード方式</option>
<option value="picture">絵</option>
<option value="youtube">Youtube</option>
<option value="naver">Naver映像</option>
...
<input type="password" name="passwd" class="span5" placeholder="パスワード" required>
...
<option selected="selected" >オプション</option>
<option value="more">畳む</option>
<option value="secret">非公開</option>
<option value="moresecret">畳む+非公開</option>
</select>
<label class="checkbox inline">
<input type="checkbox" name=op[member] value="secret">会員公開
</label>
<label class="checkbox inline">
<input type="checkbox" name="op[log]" value="adminonly">管理者のみ公開
</label>
</form>
</div>
<div class="video">
...
<textarea rows="3" class="span12" id="video" name="image" style="resize:none;margin-bottom:3px;" required placeholder="動画の <iframe> タグを入力してください。"></textarea>
<input type="password" name="passwd" class="span5" placeholder="パスワード" required>
...
<label class="checkbox inline">
<input type="checkbox" name="op[log]" value="adminonly">管理者飲み公開
</label>
</form>
</div>
<div class="text">
...
<textarea rows="3" class="span12" id="text" name="image" style="resize:none;margin-bottom:3px;" required placeholder="テキスト入力"></textarea>
<input type="password" name="passwd" class="span5" placeholder="パスワード" required>
...
<div class="span8 offset2 text-center">
	...
	<a href="./logout.php?user_id=<?=$member->user_id?>"><?=$skin->op->logout_icon?></a>
	<?php } ?>
	<a href="javascript:;" onclick="javascript:window.open('./emoticon.php?cid=<?=$cid?>','絵文字リスト','scrollbars=yes,toolbar=no,menubar=no,width=300,height=500')"><?=$skin->op->emoticon_icon?></a>
...
<div class="text-center adminpanel" >
	<p>
		<form class="adminCmtDel margin0" action="#">
			<button type="submit" id="admindelbtn" class="btn btn-mini btn-danger" style="">コメント<br/>削除</button>
		</form>
	<p/>
	<p>
		<form class="adminPicDel margin0" action="#">
			<button type="submit" id="admindelbtn" class="btn btn-mini btn-danger" style="">絵<br/>削除</button>
		</form>
	</p>
</div>
<?}else{?>
					...
					<th class="font-size user_table_background_color" colspan="2">
						<ul class="unstyled inline margin0">
							<li>
								...
								<a href="javascript:;" onclick="javascript:window.open('http://chibi.kr/twitteroauth/index.php?image=<?=$path.$pic->src?>&type=<?=$size['mime']?>','Twitterに送信','scrollbar=no,toolbar=no,menubar=no,width=510,height=170')"><img src="images/twitter-icon.png"/></a>
								<?php } ?>
								<!--// 트위터로 그림보내기 //-->
							</li>
							<li>
							...
							<!--// 로그정보 //-->
								...
								作成時間: <?=date($skin->op->time_type,$pic->time)?>
								&nbsp;|&nbsp;
								<?php }?>
								...
								function resize<?=$pic->idx?>(){
									...
									$('#resize<?=$pic->idx?>').html('(リサイズ:'+resize_w+'×'+resize_h+')');
									}else{
									$('#resize<?=$pic->idx?>').html('');
									}
								}
								...
								作成ツール: <?php if(strstr($pic->agent,"ChibiPaint")){ echo "ChibiPaint"; }else{ echo "Load"; } ?>
								<?php } ?>
								<?php if($bbs->op->showip=="all"){ echo "作成者IP: ".$pic->ip; }else if($bbs->op->showip=="admin" && $permission==ture){ echo "&nbsp;|&nbsp;作成者IP: ".$pic->ip; } else { } ?>
								&nbsp;|&nbsp;
								書き込みURL: <?php echo $path.$cid."/".$pic->no;?>
								</span>
							<!--// 로그정보 //-->
							</li>
						</ul>
...
<div id="replyForm" class="text-right">
	<p><a href="javascript:;" class="replyClose"><img alt="閉じる" src="skin/default/images/close.png"></a></p>
	<form class="margin0 cmtForm" method="POST" action="./lib/comment.submit.php" >
		...
		<div class="controls-group">
    		...
    		<input type="hidden" name="op[user_id]" value="<?=$member->user_id?>">
    		<?php if(empty($bbs->op->inst)==false){//소속아이콘이 있다면 입력폼 출력 ?>
  			<input type="text" class="input-mini" name="op[position]"  placeholder="グループ" <?php if($_COOKIE['position']) echo 'value="'.$_COOKIE['position'].'"';?> style="margin:0px !important;padding:2px;">
  			<?php }?>
  			<?php if(empty($bbs->op->inst2)==false){//소속2아이콘이 있다면 입력폼 출력 ?>
  			<input type="text" class="input-mini" name="op[position2]"  placeholder="グループ2" <?php if($_COOKIE['position2']) echo 'value="'.$_COOKIE['position2'].'"';?> style="margin:0px !important;padding:2px;">
  			<?php }?>
  			<input type="text" class="input-mini" name="name" id="name" placeholder="name" <?php if($_COOKIE['nickname']) echo 'value="'.$_COOKIE['nickname'].'"';?>>
  			<input type="password" class="input-mini" name="passwd" id="passwd" placeholder="password" <?php if($_COOKIE['passwd']) echo 'value="'.$_COOKIE['passwd'].'"';?>>
  			<?=$skin->op->write_icon?>
